...
type_I_error <- function (list_Haplotypes = list(), 
  ...
  for (i in 1:N.Sim) {
    ...
    #variable to ensure unique sub_region + 
    #minimum of NA pvalue in test +
    #maximum of loop inside the while
    #tmp_pvalue <- rep(NA, length(name_test))
    #idx_na_test <- which(is.na(tmp_pvalue))
    #nbr_same_na_test <- 0
    #min_nbr_na_test <- length(idx_na_test)
    #try_no_NA_pvalue <- 0
    #while (nbr_same_na_test < 2 && try_no_NA_pvalue < 20){
    #unique_region[i] <- NA
    ...
    #manage least minimum NA in pvalue results and decide wheter exit the while loop or not
    #if(length(which(is.na(pvalue))) > 0){
    #  #always same test that crashes
    #  if(length(which(idx_na_test %in% which(is.na(pvalue)))) == length(idx_na_test)){
    #    nbr_same_na_test <- nbr_same_na_test + 1
    #    tmp_pvalue <- pvalue
    #    tmp_computational_time <- computational_time
    #  }else{
    #    if(length(which(is.na(pvalue))) < min_nbr_na_test){
    #      idx_na_test <- which(is.na(pvalue))
    #      min_nbr_na_test <- length(idx_na_test)
    #      tmp_pvalue <- pvalue
    #      tmp_computational_time <- computational_time
    #    }
    ...
    #  tmp_computational_time <- computational_time
    #}
    #try_no_NA_pvalue <- try_no_NA_pvalue + 1
    #}#end of while pvalue is NA
