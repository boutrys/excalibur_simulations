...
data_analysis <- function(data_list = data_typeI,
  ...
  for (k in 1:nbr_experience) {
    ...
    for (i in 1:nbr_test) {
      ...
      for (j in 1:length(alpha_level)) { 
        ...
        if(nbr_non_NA_typeI > 0 & (expected_sig >= 1 | nbr_sig_typeI > 0 | j == 5)){
    ...
    for (j in 1:length(alpha_level)) {
      ...
      if(dim(tmp_data)[1] > 0){
        if(!is.null(path)){
          p <- ggplot(tmp_data, aes(x=test, y=typeIerror, fill=reliability)) + 
      ...
      if(dim(tmp_data_not_control)[1] > 0){
        if(!is.null(path)){
          p <- ggplot(tmp_data_not_control, aes(x=test, y=typeIerror, fill=reliability)) + 
            geom_bar(stat="identity") + 
            geom_hline(yintercept = y_inf, color = "blue") +
            geom_hline(yintercept = alpha_level[j], color = "red") +
            geom_hline(yintercept = y_sup, color = "blue") +
  ...
  for (i in 1:nbr_test) {
    ...
    for (j in 1:nbr_alpha) {
      ...
      if(FALSE){
        p <- ggplot(tmp_data_plot, aes(x = name, y = typeI)) +
  ...
  if(!is.null(path)){
    for (i in 1:length(alpha_level)) {
      ...
      tmp_plot <- ggplot(tmp_table, aes(test, typeIerror)) + 
