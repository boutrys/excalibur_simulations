...
ADATest <- function(genotype, phenotype, mafThr = 0.05, num_perm = 1000,
                    midp = FALSE, mode = 'additive', twoSided = TRUE) {
  data = as.matrix(genotype)
  cases = data[which(phenotype == 1),]
  ctrls = data[which(phenotype == 0),]
...
getPvalADA <- function(cases, ctrls, num_perm, midp, mode, twoSided) {
  n_cases = nrow(cases);
  n_ctrls = nrow(ctrls);
  n = n_cases + n_ctrls;
  casesori <- cases
  ...
  posit <- getScoreADAoriginal(casesori, ctrlsori, midp, mode, twoSided, optimal_threshold)
  
  return(list(pval1 = pval1, optimal_threshold = optimal_threshold, posit=posit));  
}


getScoreADA <- function(cases, ctrls, midp, mode, twoSided) {
  ...
  ww = c();
  can.threshold <- seq(0.10,0.20,0.01)
  score2v <- matrix(NA,(if (twoSided) 2 else 1),length(can.threshold))
  for (j in 1: (if (twoSided) 2 else 1)) {
    ...
    idx = attr(gp, 'index');
    if(is.matrix(gp)==F){gp <- matrix(gp,nrow=1)}
...
getScoreADAoriginal <- function(cases, ctrls, midp, mode, twoSided, optimal_threshold) {
