...
testing_stat_framework <- function(genotype_matrix = c(), 
  ...
  if(get_test){
    if(Binary){
      ...
      if(length(test) == 1){
        if(test != "all"){
        name_of_test <- name_of_test[which(name_of_test %in% test)]
        }
      }else{
        name_of_test <- name_of_test[which(name_of_test %in% test)]
      }
      if(length(name_of_test) == 0){
    ...
    if(Binary){
      ...
      qvalue <- p.adjust(pvalue[idx_Excalibur_baseline,1], method = p.adjust.methods[4]) # Bonferonni
      ...
      computational_time <- data.frame(
        computational_time = c(time_robust_burden, 
                               time_robust_SKAT,
                               time_robust_SKATO,
      ...
      errors <- data.frame(
        nbr_errors = c(err_robust_burden,
                       err_robust_SKAT,
                       err_robust_SKATO,
                       err_p_linear_liu_burden,
      ...
      }#end of else only perform robust test
      ...
      stop <- Sys.time()
      time_p_linear_davies_burden <- as.double(difftime(stop, start, units = "secs"))
      ...
      stop <- Sys.time()
      time_p_linear_weighted_liu_burden <- as.double(difftime(stop, start, units = "secs"))
      ...
      stop <- Sys.time()
      time_p_linear_weighted_liumod_skat <- as.double(difftime(stop, start, units = "secs"))
      ...
      stop <- Sys.time()
      time_p_linear_liu_skat <- as.double(difftime(stop, start, units = "secs"))
      ...
      stop <- Sys.time()
      time_p_linear_skato <- as.double(difftime(stop, start, units = "secs"))
      ...
      stop <- Sys.time()
      time_p_linear_weighted_skato <- as.double(difftime(stop, start, units = "secs"))
